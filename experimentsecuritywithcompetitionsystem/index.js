...
router.get("/", (req, res, next) => {
  res.send(
    "<html><title>Backend API system for experimenting security concept</title><body>This project provides only backend API support</body></html>"
  );
  res.end();
});
...
process.on("uncaughtException", function (error, origin) {
  ...
  console.log(
    "process.on method is automatically called for unhandled errors:\n ",
    error,
    "origin:\n",
    origin
  );
...
const sslServer = https.createServer(
  {
    key: readFileSync(
      path.join(__dirname, "src", "assets", "cert", "localhost.key")
    ),
    cert: readFileSync(
      path.join(__dirname, "src", "assets", "cert", "localhost.crt")
    ),
  },
